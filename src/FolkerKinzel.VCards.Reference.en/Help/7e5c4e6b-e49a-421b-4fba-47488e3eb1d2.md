...
`URL`: URLs pointing to websites that represent the person in some way. `(2,3,4)`
...
**C#**
``` C#
public IEnumerable<TextProperty?>? Urls { get; set; }
```
**VB**
``` VB
Public Property Urls As IEnumerable(Of TextProperty)
	Get
	Set
```
**C++**
``` C++
public:
property IEnumerable<TextProperty^>^ Urls {
...
**F#**
``` F#
member Urls : IEnumerable<TextProperty> with get, set
```
