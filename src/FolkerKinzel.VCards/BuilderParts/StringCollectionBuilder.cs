...
public readonly struct StringCollectionBuilder
{
    ...
    public VCardBuilder Edit(Action<IEnumerable<StringCollectionProperty>> action)
    {
        var props = Builder.VCard.Get<IEnumerable<StringCollectionProperty?>?>(_prop)?.WhereNotNull() ?? [];
