...
public class V3Tests
{
    ...
    {
        IList<VCard>? vcard = VCard.LoadVcf(TestFiles.V3vcf).ToList();
    ...
    {
        IList<VCard>? vcard = VCard.LoadVcf(@"C:\Users\fkinz\OneDrive\Kontakte\Thunderbird\21-01-13.vcf").ToList();
    ...
    {
        IList<VCard>? vcard = VCard.LoadVcf(TestFiles.PhotoV3vcf).ToList();
    ...
    {
        var vc = VCard.LoadVcf(TestFiles.PhotoV3vcf).ToList();
        vc.Add(vc[0]);

        string s = vc.ToVcfString();

        vc = VCard.ParseVcf(s).ToList();
...
END:VCARD";
        ...
        VCard vcard = VCard.ParseVcf(vcardString).First();
